...
void packet_channel_send_data(struct buf *b) {
    ...
    crypto_uint32_store_bigendian(
        b->buf + 1, channel_getid()); /* uint32    recipient channel    */
...
void packet_channel_send_extendeddata(struct buf *b) {
    ...
    b->len = r + 13;
    b->buf[0] =
        SSH_MSG_CHANNEL_EXTENDED_DATA; /* byte SSH_MSG_CHANNEL_EXTENDED_DATA */
    crypto_uint32_store_bigendian(
        b->buf + 1, channel_getid()); /* uint32    recipient channel */
    crypto_uint32_store_bigendian(
        b->buf + 5, 1); /* uint32    data_type_code (1 = stderr)   */
...
int packet_channel_send_close(struct buf *b, int exitsignal, int exitcode) {
    ...
    if (exitsignal > 0) {
        buf_purge(b);
        buf_putnum8(b,
                    SSH_MSG_CHANNEL_REQUEST); /* byte SSH_MSG_CHANNEL_REQUEST */
        ...
        buf_putstring(b,
                      ssh_sigstr(exitsignal)); /* string    signal name (without
                                                  the "SIG" prefix) */
        buf_putnum8(b, 0);                     /* boolean   core dumped */
        buf_putstring(
            b, ssh_sigstrlong(exitsignal)); /* string    error message in
